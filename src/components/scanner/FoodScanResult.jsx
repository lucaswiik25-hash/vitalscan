...
import { ArrowLeft, Sparkles, Loader2, Plus, Pencil } from 'lucide-react';
...
const PillBadge = ({ value }) => {
  ...
  const colors = {
    ...
    damages: 'bg-red-100 text-red-700',
...
export default function FoodScanResult({ result, onLog, onLogAnalysisOnly, onScanAnother, onBack }) {
  ...
  const analyzeIngredients = async () => {
    ...
    const { data: r } = await base44.functions.invoke('analyzeWithClaude', {
      prompt: `You are a food scientist and toxicologist. Analyze every ingredient in this product.
Product: "${result.name}"
Ingredients: "${result.ingredients_text || 'Not available — analyze based on typical ingredients for this product type'}"
For each: name, body_effect (1 sentence), safety_rating ("Green"/"Yellow"/"Red"), is_seed_oil, is_hidden_sugar, is_emulsifier, is_hormone_disruptor, is_allergen, allergen_name, is_artificial_sweetener, is_preservative, is_artificial_color, is_whole_food.
Also: overall_verdict ("Clean"/"Mostly Clean"/"Mixed"/"Mostly Processed"/"Avoid"), verdict_reason. NEVER fail.`,
  ...
  // Slide 0: Macros
  const slide0 = (
    <div className="space-y-3 pb-4 fade-in-up">
      <div className="bg-gray-50 rounded-2xl p-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {result.health_score && (
            <span className="ml-1 px-2 py-0.5 rounded-full text-xs font-bold bg-gray-800 text-white">{result.health_score}/10</span>
          )}
        </div>
      </div>
      <div className="grid grid-cols-3 gap-2">
        {[{ emoji: '🍖', label: 'Protein', value: `${prot}g` }, { emoji: '🌾', label: 'Carbs', value: `${carbs}g` }, { emoji: '🫒', label: 'Fats', value: `${fat}g` }].map(m => (
  ...
  // Slide 1: Body / Appearance verdict
  ...
  const slide2 = result.is_appearance_mode ? (
    <div className="pb-4 space-y-3 fade-in-up">
      ...
      <div className="bg-gray-50 rounded-2xl p-4 space-y-2">
        {[['Calories', `${cal} kcal`], ['Protein', `${prot}g`], ['Carbs', `${carbs}g`], ['Fat', `${fat}g`], ['Sugar', `${Math.round((result.sugar || 0) * servings * 10) / 10}g`], ['Sodium', `${Math.round((result.sodium || 0) * servings)}mg`]].map(([l, v]) => (
    ...
    <div className="fixed inset-0 bg-white flex flex-col" style={{ maxWidth: 480, margin: '0 auto' }}>
      ...
      <div className="shrink-0 flex items-center justify-between px-4 pt-12 pb-3 border-b border-gray-100 fade-in-up">
        ...
        <div className="text-center">
          <p className="text-sm font-bold text-gray-900 max-w-[200px] truncate">{result.name}</p>
          <p className="text-xs text-gray-400">{result.serving_size || 'per serving'}</p>
        </div>
        {/* Servings adjuster */}
        <div className="flex items-center gap-1 border border-gray-200 rounded-2xl px-2.5 py-1.5">
          <button onClick={() => setServings(s => Math.max(0.5, s - 0.5))} className="text-gray-400 text-sm font-bold w-5 text-center">−</button>
          <span className="text-sm font-semibold text-gray-800 px-1">{servings}×</span>
          <button onClick={() => setServings(s => s + 0.5)} className="text-gray-400 text-sm font-bold w-5 text-center">+</button>
        </div>
      </div>
      ...
      >
        ...
        >
          {allSlides.map((s, i) => (
            <div key={i} className="min-w-full h-full overflow-y-auto px-4 pt-2">
              {s}
            </div>
          ))}
        </div>
